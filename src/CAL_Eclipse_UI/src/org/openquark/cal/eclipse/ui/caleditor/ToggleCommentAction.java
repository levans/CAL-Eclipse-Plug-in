...
public final class ToggleCommentAction extends TextEditorAction {
    ...
    public void run() {
        ...
        if (isSelectionCommented(getSelection())) {
        ...
        if (!fOperationTarget.canDoOperation(operationCode)) {
            if (shell != null) {
                MessageDialog.openError(shell, CALEditorMessages.ToggleComment_error_title, CALEditorMessages.ToggleComment_error_message);
            }
            return;
        }
    ...
    private Shell getShell() {
        ...
        } else if (fOperationTarget instanceof ISourceViewer) {
            return ((ISourceViewer) fOperationTarget).getTextWidget().getShell();
        } else {
            return null;
        }
    }
    ...
    private IDocument getDocument() {
        if (fOperationTarget instanceof ISourceViewer) {
            return ((ISourceViewer) fOperationTarget).getDocument();
        } else if (getTextEditor() != null) {
    ...
    private int getFirstCompleteLineOfRegion(IRegion region, IDocument document) {
        ...
        try {
            ...
            // should not happen
            // But unfortunately can happen with Eclipse 3.2.x and earlier if the last line is
            // t2 = error "hi"
...
//            CALEclipseUIPlugin.log(x);
    ...
    private ISelection getSelection() {
        if (fOperationTarget instanceof ISourceViewer) {
            return ((ISourceViewer) fOperationTarget).getSelectionProvider().getSelection();
        } else if (getTextEditor() != null) {
