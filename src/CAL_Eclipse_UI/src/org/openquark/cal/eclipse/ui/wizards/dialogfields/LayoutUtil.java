...
/**
 * Static helper methods used in laying out dialog fields.
 * @author Edward Lam
 */
public class LayoutUtil {
    ...
    public static int getNumberOfColumns(DialogField[] editors) {
        int nCulumns = 0;
        for (final DialogField dialogField : editors) {
            nCulumns = Math.max(dialogField.getNumberOfControls(), nCulumns);
        }
        return nCulumns;
    }
    ...
    private static void modifyLabelSpans(Control[][] controls, int nColumns) {
        for (final Control[] control : controls) {
            setHorizontalSpan(control[0], nColumns);
        }
    }
