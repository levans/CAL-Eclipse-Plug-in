...
/**
 * CAL specific change object.
 */
public abstract class CALChange extends Change {
            ...
            if (fDirty) {
                if (fKind == DOCUMENT && fTextFileBuffer != null && stampToMatch == fModificationStamp) {
                    ...
                    status.addFatalError(Messages.format(
                        CALUIMessages.Change_is_unsaved, fResource.getFullPath().toString())); 
                }
            }
        }
        ...
        public void checkModificationStamp(RefactoringStatus status, long stampToMatch) {
            if (fKind == DOCUMENT) {
                if (stampToMatch != IDocumentExtension4.UNKNOWN_MODIFICATION_STAMP && fModificationStamp != stampToMatch) {
                    status.addFatalError(Messages.format(
                        CALUIMessages.Change_has_modifications, fResource.getFullPath().toString())); 
                }
            } else {
    ...
    protected static void checkExistence(RefactoringStatus status, Object element) {
        if (element == null) {
            status.addFatalError(CALUIMessages.DynamicValidationStateChange_workspace_changed); 
            
        } else if (element instanceof IResource && !((IResource)element).exists()) {
            status.addFatalError(Messages.format(
                CALUIMessages.Change_does_not_exist, ((IResource)element).getFullPath().toString())); 
        } else if (element instanceof IJavaElement && !((IJavaElement)element).exists()) {
            status.addFatalError(Messages.format(
                CALUIMessages.Change_does_not_exist, ((IJavaElement)element).getElementName())); 
        }
    }
