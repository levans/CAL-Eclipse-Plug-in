 ...
 *
 * Contributors:
 *     IBM Corporation - initial API and implementation. Unmodified copy of org.eclipse.jdt.internal.ui.text.java.hover.BrowserInformationControl
 *******************************************************************************/
 ...
 * widget.
 * 
 * This is used to display CALDoc in a text hover
 *
 * <p>
...
public class BrowserInformationControl implements IInformationControl, IInformationControlExtension, IInformationControlExtension3,  DisposeListener {
     ...
     */
    static class BorderFillLayout extends Layout {
    ...
    public Point computeSizeHint() {
        final Point computedSize = fShell.computeSize(SWT.DEFAULT, SWT.DEFAULT);
        return new Point(Math.max(computedSize.x, 500), Math.max(computedSize.x, 200)); // we need to force a minimum size of (500, 20) for the tooltips
    }
            ...
            fDeactivateListener = new Listener() {
                public void handleEvent(Event event) {
                    ...
                    for (final Object listener : listeners) {
                        ((FocusListener)listener).focusLost(new FocusEvent(event));
                    }
                }
            };
