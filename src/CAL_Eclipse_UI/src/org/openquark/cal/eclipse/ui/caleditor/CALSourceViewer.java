...
/**
 * A ProjectionViewer for CAL source.
 * @author Edward Lam
 */
public class CALSourceViewer extends ProjectionViewer implements IPropertyChangeListener {
    ...
    public CALSourceViewer(Composite parent, IVerticalRuler verticalRuler, IOverviewRuler overviewRuler, boolean showAnnotationsOverview, int styles, IPreferenceStore store) {
...
//        return context;
    ...
    /**
     * Overrides the default offset in the information presented. This is used so that
     * if a hover is showing and F2 is pressed that the location of the hover will be 
     * used instead of the location of the cursor.
     * 
     * @param offset The override value for the offset.
     */
    public void setOffset(int offset){
        if (fInformationPresenter != null){
            InformationPresenter ip = (InformationPresenter) fInformationPresenter;
            ip.setOffset(offset);
        }
    }
    /*
    ...
    public void configure(SourceViewerConfiguration configuration) {
        ...
        if (configuration instanceof CALSourceViewerConfiguration) {
            ...
            fStructurePresenter = calSVCconfiguration.getOutlinePresenter(this, true);
            if (fStructurePresenter != null) {
                fStructurePresenter.install(this);
            }

            fHierarchyPresenter = calSVCconfiguration.getHierarchyPresenter(this, true);
            if (fHierarchyPresenter != null) {
    ...
    protected void initializeViewerColors() {
        if (fPreferenceStore != null) {
            ...
            // ----------- foreground color --------------------
            Color color = fPreferenceStore.getBoolean(AbstractTextEditor.PREFERENCE_COLOR_FOREGROUND_SYSTEM_DEFAULT) ? null : 
            ...
            color = fPreferenceStore.getBoolean(AbstractDecoratedTextEditorPreferenceConstants.EDITOR_SELECTION_FOREGROUND_DEFAULT_COLOR) ? null : 
            ...
            color = fPreferenceStore.getBoolean(AbstractDecoratedTextEditorPreferenceConstants.EDITOR_SELECTION_BACKGROUND_DEFAULT_COLOR) ? null : 
    ...
    /**
     * Creates a color from the information stored in the given preference store. Returns <code>null</code> if there
    ...
    public void prependTextPresentationListener(ITextPresentationListener listener) {
        ...
        List<Object> tpl = UnsafeCast.unsafeCast(fTextPresentationListeners);
        tpl.add(0, listener);
    }
    ...
    protected static int[] getBidiLineSegments(IDocument document, int lineOffset) throws BadLocationException {
        ...
        ITypedRegion[] linePartitioning = TextUtilities.computePartitioning(document, CALPartitions.CAL_PARTITIONING, lineOffset, line.getLength(), false);

        List<ITypedRegion> segmentation = new ArrayList<ITypedRegion>();
        for (final ITypedRegion region : linePartitioning) {
            if (CALPartitions.CAL_STRING.equals(region.getType())) {
                segmentation.add(region);
            }
        }
     ...
     * <p>
     * This is a performance optimization to reduce the computation of the text presentation triggered by
     * <code>setVisibleDocument(IDocument)</code>.
     * </p>
     * 
    ...
    public AbstractInformationControlManager getTextHoveringController2(){
        return getTextHoveringController();
    }
